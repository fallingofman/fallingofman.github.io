...
def pull_meme_data(client, meme_page_usernames):
	...
	for username in meme_page_usernames:
		...
		user_media_map[username] = cl.user_medias(user_id, 20)
...
def upload_videos(client):
	...
	if len(files) > 0:
		...
		cl.clip_upload(os.path.dirname(__file__) + "/" + next_file_upload_name, "hello this is a test from instagrapi")
